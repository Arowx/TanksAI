...
namespace Complete
{
    ...
    {
        public int m_TimeLimit = 60;

        private float m_EndOfRoundTime;
        ...
        private IEnumerator GameLoop ()
        {
            ...
            if (m_GameWinner != null || m_RoundNumber >= m_NumRoundsToWin * 2f)
            {
                m_RoundNumber = 0;

                int scoreTankA = m_Tanks[0].m_Wins * 3 + m_Tanks[0].m_Draws;
                int scoreTankB = m_Tanks[1].m_Wins * 3 + m_Tanks[1].m_Draws;

                m_Tanks[0].DestoryTank();
                m_Tanks[1].DestoryTank();

                cm.EndGame(scoreTankA, scoreTankB);
        ...
        private IEnumerator RoundPlaying ()
        {
            ...
            while (!OneTankLeft() && (Time.timeSinceLevelLoad <= m_EndOfRoundTime))
            {
        ...
        {
            if (OneTankLeft()) // one winner else draw
            {
                // Go through all the tanks...
                for (int i = 0; i < m_Tanks.Length; i++)
                {
